...
describe('Database', () => {
  ...
  it('Finding document', async () => {
    ...
    const projectedDoc = await db.findOne(
      { _id: insertedDoc._id },
      { data: 1, _id: 0 },
    );
  ...
  it('Updating documents with options', async () => {
    ...
    let numberOfUpdatedDocs = await db.update(
      { update: true },
      { $set: { data: 'First update' } },
      { multi: true },
    );
    ...
    const upsertedDoc = (await db.update(
      { update: true, data: 'First update' },
      { $set: { data: 'Third update' } },
      { upsert: true },
    )) as Document;
    ...
    numberOfUpdatedDocs = await db.update(
      { data: 'Third update' },
      { $set: { data: 'Fourth update' } },
      { upsert: true },
    );
  ...
  it('Finding documents', async () => {
    ...
    const insertedDoc1 = (await db.insert({
      data: data1,
      flag: true,
      no: 1,
    })) as Document;
    const insertedDoc2 = (await db.insert({
      data: data2,
      flag: true,
      no: 2,
    })) as Document;
    ...
    const projectedDocs = (await db.find(
      { flag: true },
      { no: 1, _id: 0 },
    )) as Array<Document>;
  ...
  it('Test invalid database queries', async () => {
    ...
    try {
      ...
      expect((err as Error).message).to.equal(
        'Unknown comparison function $invalidComparator',
      );
    }
    ...
    try {
      ...
      expect((err as Error).message).to.equal(
        'Unknown comparison function $undefinedComparator',
      );
    }
